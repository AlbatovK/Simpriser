...
public class SpringApplicationTests {
    ...
    private static SpringApplicationTests testClass = null;

    @Before
    public void setUp() throws Exception {
        if (testClass == null) {
            quizzes.clear();
            quizzes.addAll(service.getQuizzes());
    ...
    public static void cleanUpAfterAll() throws Exception {
        testClass.cleanUpAfterTests();
    }

    private void cleanUpAfterTests() throws Exception {
        for (Quiz q : service.getQuizzes())
            service.deleteQuiz(q);
        for (Quiz q : quizzes)
            service.saveQuiz(q);
        System.out.println("Repopulate success");
    }
